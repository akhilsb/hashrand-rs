...
pub async fn start_batchwss(cx: &mut Context)-> Vec<CoinMsg>{
...
pub async fn process_batchwss_init(cx: &mut Context, wss_init: BatchWSSMsg, ctr: CTRBCMsg, smr_msg:&mut SMRMsg) {
    let now = SystemTime::now();
    ...
    smr_msg.coin_msg = CoinMsg::BatchWSSEcho(ctr.clone(), master_merkle_root,cx.myid);
