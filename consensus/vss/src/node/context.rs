...
use std::sync::Arc;
use types::appxcon::{Replica, WrapperMsg};
use vsss_rs::Share;
...
pub struct Context {
    ...
    pub sec_share_map: HashMap<Replica, Share<33>>,
...
impl Context {
    pub fn new(config: &Node, net_send: UnboundedSender<(Replica, Arc<WrapperMsg>)>) -> Self {
        ...
        if v[0] == "a" {
            ...
            log::debug!(
                "Started n-parallel RBC with value {:?} and epsilon {}",
                c.value,
                c.epsilon
            );
            // Initialize storage
