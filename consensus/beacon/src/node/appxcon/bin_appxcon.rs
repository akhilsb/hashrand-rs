...
impl Context{
    ...
    pub async fn process_baa_echo(self: &mut Context, msgs: Vec<(Round,Vec<(Replica,Vec<u8>)>)>, echo_sender:Replica, round:Round){
        ...
        for (round_iter,values) in msgs.into_iter(){
            ...
            if rbc_state.round_state.contains_key(&round){
                ...
                if rnd_state.term_vals.len() == self.num_nodes {
                    log::info!("All n instances of Binary AA terminated for round {}, checking for termination related to round {}",round,round_iter);
        ...
        if send_valmap_echo1.len() > 0{
            ...
            self.broadcast(CoinMsg::BinaryAAEcho(vec_transmit.clone(), self.myid, round)).await;
        ...
        if send_valmap_echo2.len() > 0{
            ...
            self.broadcast(CoinMsg::BinaryAAEcho2(vec_transmit.clone(), self.myid, round)).await;
    ...
    pub async fn next_round_begin(&mut self,round:Round){
        ...
        if round_begin > 1 && self.round_state.contains_key(&(round_begin-1)) && self.round_state.get(&(round_begin-1)).unwrap().round_state.contains_key(&(round-1)){
            ...
            let mapped_rvecs:Vec<(Replica,BigInt)> = 
                rbc_state.round_state.get(&(round_begin-1)).unwrap().term_vals.clone().into_iter()
                .filter(|(_rep,num)| *num > BigInt::from(0i32))
                .collect();
            for (rep,val) in mapped_rvecs.into_iter(){
                rbc_state.nz_appxcon_rs.insert(rep, (val,false,BigInt::from(0i32)));
            }
            log::info!("Terminated beacon for round {}", round_begin-1);
            ...
            self.reconstruct_beacon(round_begin-1, 0).await;
        }
