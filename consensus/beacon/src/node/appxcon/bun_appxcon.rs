    ...
    pub async fn next_round_vals(&mut self, round:Round)->HashMap<Round,Vec<(Replica,Val)>>{
        ...
        for node in 0..self.num_nodes{
            if rbc_state.appxcon_allround_vals.contains_key(&node){
                ...
                for (round,vec_value) in values_replica.clone().into_iter(){
                    if hmap.contains_key(&round){
                        let index_map = hmap.get_mut(&round).unwrap();
        ...
        for (term_round,round_vecs) in term_rounds.into_iter(){
            ...
            let appxcon_map = &mut rbc_iter_state.nz_appxcon_rs;
            ...
            self.reconstruct_beacon(term_round, 0).await;
        }
        return return_map;
    }
}