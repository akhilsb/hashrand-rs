...
use types::appxcon::{CTRBCMsg, ProtMsg, Replica, WrapperMsg};

use crate::node::{
    check_for_ht_witnesses, handle_witness, reconstruct_and_return, reconstruct_and_verify,
    verify_merkle_proof, RoundState,
};
...
pub async fn process_ready(cx: &mut Context, ctr: CTRBCMsg, ready_sender: Replica) {
    ...
    log::info!(
        "Received READY message from {} for RBC of node {}",
        ready_sender,
        rbc_origin
    );
    let round = 0;
    if !verify_merkle_proof(&mp, &shard) {
        log::error!(
            "Failed to evaluate merkle proof for READY received from node {} for RBC {}",
            ready_sender,
            rbc_origin
        );
        return;
    ...
    if round_state_map.contains_key(&round) {
        ...
        if merkle_root != mp.to_proof().root() {
            log::error!(
                "Merkle root verification failed with error {:?}{:?}",
                merkle_root,
                mp.to_proof().root()
            );
        }
        ...
        log::debug!(
            "READY check: Round equals: {}, echos.len {}, contains key: {}",
            cx.round == round,
            readys.len(),
            ...
            && !readys.contains_key(&cx.myid)
        {
            ...
            match res {
                Err(error) => log::error!(
                    "Shard reconstruction failed because of the following reason {:?}",
                    error
                ),
                Ok(vec_x) => {
        ...
        rnd_state
            .node_msgs
            .insert(rbc_origin, (shard.clone(), mp.clone(), mp.root()));
        let mut readyset = HashMap::default();
        readyset.insert(ready_sender, (shard.clone(), mp.clone()));
        rnd_state.readys.insert(rbc_origin, readyset);
...
pub async fn process_reconstruct_message(cx: &mut Context, ctr: CTRBCMsg, recon_sender: Replica) {
    ...
    // Highly unlikely that the node will get an echo before rbc_init message
    log::info!(
        "Received Reconstruct message from {} for RBC of node {}",
        recon_sender,
        rbc_origin
    );
    let round = 0;
    if !verify_merkle_proof(&mp, &shard) {
        log::error!(
            "Failed to evaluate merkle proof for RECON received from node {} for RBC {}",
            recon_sender,
            rbc_origin
        );
        return;
    ...
    if round_state_map.contains_key(&round) {
        ...
        let ready_check =
            rnd_state.readys.get(&rbc_origin).unwrap().len() >= (cx.num_nodes - cx.num_faults);
        let vec_fmap = rnd_state.recon_msgs.get(&rbc_origin).unwrap();
        if vec_fmap.len() >= cx.num_nodes - cx.num_faults && ready_check {
            // Reconstruct here
            let result =
                reconstruct_and_return(vec_fmap, cx.num_nodes.clone(), cx.num_faults.clone());
            match result {
                Err(error) => {
                    log::error!(
                        "Error resulted in constructing erasure-coded data {:?}",
                        error
                    );
                    return;
                }
                Ok(vec) => {
                    ...
                    log::info!(
                        "Successfully reconstructed message for RBC, terminating RBC of node {}",
                        rbc_origin
                    );
                    log::info!(
                        "Terminated with message: {:?}",
                        String::from_utf8(vec.clone()).expect("Invalid utf8")
                    );
                    ...
                    if rnd_state.terminated_rbcs.len() >= cx.num_nodes - cx.num_faults {
                        if !rnd_state.witness_sent {
                            ...
                            log::info!("Round state: {:?}", rnd_state.terminated_rbcs);
                            let vec_rbcs =
                                Vec::from_iter(rnd_state.terminated_rbcs.clone().into_iter());
                            let witness_msg = ProtMsg::WITNESS(vec_rbcs.clone(), cx.myid, cx.round);
                            msgs_to_be_sent.push(witness_msg);
                            rnd_state.witness_sent = true;
                        }
