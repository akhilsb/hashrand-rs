    ...
    pub async fn start_batchwss(&mut self){
        ...
        for (rep,batch_wss) in vec_msgs_to_be_sent.iter_mut(){
            ...
            if replica != self.myid{
                ...
                batch_wss.master_root = master_root.clone();
                self.process_batchwss_init(batch_wss.clone(),ctrbc_msg).await;
            }
        }
        self.add_benchmark(String::from("start_batchwss"), now.elapsed().unwrap().as_nanos());
    }
    
    pub async fn process_batchwss_init(self: &mut Context, wss_init: BatchWSSMsg, ctr: CTRBCMsg) {
        let now = SystemTime::now();
        ...
        let wss_state = &mut self.batchvss_state;
        ...
        self.broadcast(CoinMsg::BatchWSSEcho(ctr.clone(), master_merkle_root,self.myid)).await;
